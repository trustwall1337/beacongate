...
- **Traffic-pattern analysis** — long-poll cadence, payload-size
  ...
  bytes are encrypted.
- **TLS-fingerprint analysis (JA3 / JA4)** — **Reduced, not closed.**
  The `appsscript` transport uses
  ...
  probing, traffic analysis, Google-side classification, or
  future fingerprinting methods.** See
  ...
  partial mitigation. (b) Active probing (a censor that connects
  to `script.google.com` paths to verify the response shape) is
  a live category of attack uTLS does not address. (c) New
  fingerprinting techniques beyond JA3/JA4 could emerge.
  (d) Compile-time tampering or a forced downgrade to stdlib `tls`
  via build tags re-opens the gap — operators relying on the
  disguise should verify the emitted ClientHello on their built
  binary.
